  ...
  return (
    <div className="p-4 space-y-6 animate-in fade-in duration-500">
      <div className="space-y-4 border-b border-border/50 pb-4">
        <Link
          href="/"
          data-testid="button-exit-oracle-control"
          aria-label="Вийти з контролю Оракула"
          className="inline-flex h-10 items-center gap-2 rounded-full border border-primary/30 bg-secondary/70 px-4 text-xs font-semibold uppercase tracking-[0.16em] text-foreground/85 transition-colors hover:border-primary/70 hover:bg-primary/15 hover:text-primary"
        >
...
function StatCard({
  ...
  return (
    <div
      className={`p-4 bg-card border ${danger ? "border-destructive/50 bg-destructive/10" : highlight ? "border-primary/50" : "border-border"} flex flex-col gap-2`}
    >
      <div className="flex items-center gap-2 text-muted-foreground">
        <Icon
          className={`w-4 h-4 ${danger ? "text-destructive" : highlight ? "text-primary" : ""}`}
        />
      ...
      <div
        className={`font-serif text-2xl font-bold ${danger ? "text-destructive" : highlight ? "text-primary" : "text-foreground"}`}
      >
        {value}
      </div>
    </div>
